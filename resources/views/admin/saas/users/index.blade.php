<x-layouts.saas title="Manage Users">
    ...
    <style>
        ...
        .user-avatar { width: 36px; height: 36px; border-radius: 999px; background: #e0e7ff; color: #4f46e5; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 14px; }
        .tenant-badge { background: #f1f5f9; color: #475569; padding: 4px 10px; border-radius: 6px; font-size: 12px; font-weight: 600; display: inline-flex; align-items: center; gap: 4px; }
    ...
    <div class="page-header">
        <div style="width:48px;height:48px;background:linear-gradient(135deg, #e0e7ff, #c7d2fe);border-radius:12px;display:flex;align-items:center;justify-content:center;color:#4f46e5;font-size:24px;box-shadow:inset 0 2px 4px rgba(255,255,255,0.5);">
            <i class="bi bi-people-fill"></i>
        </div>
        <div>
            <h1>Global Users</h1>
            <p>Manage and monitor all users across all bakery tenants.</p>
        </div>
    </div>

    <div class="card">
        <div class="card-header" style="display:flex; align-items:center;">
            <i class="bi bi-person-lines-fill" style="color:#6366f1;font-size:18px;margin-right:8px;"></i>
            ...
            <form method="GET" action="{{ route('saas.users.index') }}" class="filter-bar" style="margin-left:auto;margin-bottom:0;">
                ...
                @if(request()->has('search'))
                    <a href="{{ route('saas.users.index') }}" class="btn btn-outline btn-sm"><i class="bi bi-x-lg"></i> Clear</a>
                @endif
            </form>
        </div>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="text-align:right;">
                            <div style="display:flex; gap:8px; justify-content:flex-end;">
                                <a href="{{ route('saas.users.show', $user->id) }}" class="btn btn-outline btn-sm">
                                    <i class="bi bi-eye"></i> Details
                                </a>
                                <form action="{{ route('saas.impersonate', $user->id) }}" method="POST" style="margin:0;">
                                    @csrf
                                    <button type="submit" class="btn btn-primary btn-sm" onclick="return confirm('Log in as {{ $user->name }}?')">
                    ...
                    <tr>
                        <td colspan="5" style="text-align:center;padding:40px;color:#64748b;">
                            <i class="bi bi-people" style="font-size:32px;color:#cbd5e1;display:block;margin-bottom:12px;"></i>
